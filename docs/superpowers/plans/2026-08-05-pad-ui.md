...
**Architecture:** 用 material3-adaptive 的 `WindowSizeClass` 做两档断点（compact/medium 走手机布局，expanded 走 PAD 布局）。每个页面新增 tablet 分支私有 composable，与手机分支互不干扰。数据层零改动；新增纯函数排序工具（可单测）。导航结构：MainScreen 顶部全局栏（logo+搜索）+ 左侧 NavigationRail（设置置底），5 个 tab 的页面在 tablet 下隐藏自带 TopAppBar 避免双栏。

**Tech Stack:** Kotlin 2.1.20, Jetpack Compose (BOM 2024.12.01), material3-adaptive, Navigation Compose, Hilt

参考 spec：`docs/superpowers/specs/2026-08-05-pad-ui-design.md`

---

### Task 1: 依赖 + 断点工具 + 字符串资源
...
- [ ] **Step 1: toml 加 material3-adaptive 依赖（BOM 管理，不写版本号）**

`gradle/libs.versions.toml` 的 `[libraries]` 段，在 `androidx-compose-material3` 行后加一行：

```toml
androidx-compose-material3-adaptive = { group = "androidx.compose.material3", name = "material3-adaptive" }
```

- [ ] **Step 2: build.gradle.kts 加依赖**

`app/build.gradle.kts` 的 `// Compose` 依赖块，`implementation(libs.androidx.compose.material3)` 行后加：

```kotlin
implementation(libs.androidx.compose.material3.adaptive)
```

- [ ] **Step 3: 新建断点工具 AppLayout.kt**
...
fun rememberIsTablet(): Boolean {
    ...
    return info.windowSizeClass.widthSizeClass == WindowSizeClass.WidthSizeClass.Expanded
}
```
...
Expected: BUILD SUCCESSFUL（如 material3-adaptive 未随 BOM 解析，则在 toml 该行加显式版本 `version = "1.0.0"` 重试）

- [ ] **Step 6: Commit**

```bash
git add gradle/libs.versions.toml app/build.gradle.kts app/src/main/java/com/dream/wowiptv/presentation/common/AppLayout.kt app/src/main/res/values/strings.xml app/src/main/res/values-en/strings.xml
git commit -m "feat: material3-adaptive依赖+rememberIsTablet断点工具+排序/海报尺寸字符串"
```
